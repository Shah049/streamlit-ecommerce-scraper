...
class StreamlitEcommerceScraper:
    ...
    def extract_price(self, soup):
        # Method 1: Look for meta tags with price content (often the most reliable)
        ...
        # Method 2: Use the standard list of visible text selectors
    ...
    def extract_compatible_brands(self, description):
        ...
        # Build a regex pattern from our brand list to find matches
    ...
    def extract_product_data(self, product_url):
        ...
        # Combine description and product details for a more comprehensive brand search
    ...
    if st.button("🚀 Start Scraping", type="primary", use_container_width=True):
        ...
        try:
            ...
            if products:
                ...
                st.success(f"✅ Scraped {len(products)} products in {duration:.2f} seconds!")
                df = pd.DataFrame(products)

                # --- NEW: Drop columns that are entirely empty ---
                df.replace('', pd.NA, inplace=True)
                df.dropna(axis=1, how='all', inplace=True)
                df.fillna("", inplace=True)  # Fill back for display
                # Intelligent column ordering, now including product_details
                ...
                st.markdown(create_download_link(df, filename, file_format.lower()), unsafe_allow_html=True)
            else:
                st.warning(
